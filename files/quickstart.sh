...
if [ $DELETE_DEPLOYMENT == "true" ]; then
   ...
   /opt/workspace/files/deploy-quarkusrobotshop-ansible.sh  -d ${CLUSTER_DOMAIN_NAME} -t  ${TOKEN}  -s STORE-${RANDOM_STRING} -u true
else 
    /opt/workspace/files/deploy-quarkusrobotshop-ansible.sh  -d ${CLUSTER_DOMAIN_NAME} -t  ${TOKEN}  -s STORE-${RANDOM_STRING}
fi
